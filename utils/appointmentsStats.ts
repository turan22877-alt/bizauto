...
export function confirmedRevenueByMonth(appointments: Appointment[]): { key: string; revenue: number; label: string }[] {
...
export function appointmentsCountByMonth(appointments: Appointment[]): { key: string; count: number; label: string }[] {
  const map = new Map<string, number>();
  for (const a of appointments) {
    const dt = parseAppointmentDate(a.date);
    if (!dt) continue;
...
export function revenueInMonth(appointments: Appointment[], year: number, month0: number): number {
  return appointments
    .filter((a) => {
