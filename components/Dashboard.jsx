...
const Dashboard = ({ clients, appointments, onNavigate }) => {
  const confirmed = useMemo(
    () => appointments.filter((a) => a.status === "confirmed"),
    [appointments],
  );
  const totalRevenue = useMemo(
    () => confirmed.reduce((sum, app) => sum + (app.price || 0), 0),
    [confirmed],
  );
    ...
    <div className="space-y-10">
        ...
        <div className="xl:col-span-7 space-y-6">
          <div className="flex flex-col sm:flex-row sm:justify-between sm:items-end gap-4">
            <div>
              <h2 className="header-font text-3xl md:text-4xl font-black text-slate-800 tracking-tight flex items-center gap-3">
                Обзор
                <Sparkles
                  className="text-orange-600 hidden sm:inline"
                  size={28}
                />
              </h2>
            </div>
          ...
          <div className="flex flex-wrap gap-3">
            ...
            <Button
              ...
              onClick={() => onNavigate(AppSection.ANALYTICS)}
            >
              Аналитика
            </Button>
            <Button
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-5 md:gap-6">
        {stats.map((stat, i) => (
          ...
          >
            ...
            />
            <div
              className={`absolute top-0 right-0 w-28 h-28 opacity-[0.07] translate-x-6 -translate-y-6 group-hover:scale-110 transition-transform duration-500`}
            >
              <stat.icon size={112} className={stat.iconClass} />
            </div>
            <p className="text-[10px] font-black text-slate-600 uppercase tracking-widest mb-2 relative">
              {stat.label}
            </p>
            <h3 className="text-2xl md:text-3xl font-black text-slate-800 relative">
              {stat.value}
            </h3>
          </div>
        ))}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 lg:gap-8">
        <div className="glass-panel card-3d p-8 md:p-10 rounded-[2rem] border border-slate-200">
          <h3 className="header-font text-lg font-bold mb-6 text-slate-800">
            Ближайшие события
          </h3>
                  ...
                  >
                    <div className="flex items-center gap-4 min-w-0">
                      ...
                      <div className="min-w-0">
                        ...
                        <p className="text-[10px] text-slate-600 uppercase font-bold truncate">
                          {app.service}
                        </p>
                      </div>
                    </div>
                    <p className="text-xs font-black text-orange-600 shrink-0 ml-2">
                      {app.startTime}
                    </p>
                  </div>
                ))}
        ...
        <div className="glass-panel card-3d p-8 md:p-10 rounded-[2rem] border border-stone-200 bg-gradient-to-br from-orange-600/[0.08] via-transparent to-orange-600/[0.06]">
          ...
          <p className="text-sm text-slate-600 mb-8">
            Доля подтверждённых среди всех записей в журнале
          </p>

          {completionRate === null ? (
            <p className="text-slate-600 text-sm font-medium">
              Добавьте записи в журнале — здесь появится доля подтверждённых.
            </p>
          ) : (
