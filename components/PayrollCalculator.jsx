  ...
  return (
    <div className="space-y-10">
      <div className="flex flex-col sm:flex-row sm:justify-between sm:items-end gap-4">
        <div>
          <h2 className="header-font text-3xl md:text-4xl font-black text-slate-800 mb-2">Зарплаты</h2>
      ...
      <div className="glass-panel p-10 md:p-12 rounded-[2.5rem] border border-slate-200 text-center relative overflow-hidden">
        ...
        <p className="text-slate-600 max-w-md mx-auto font-medium mb-8 text-sm">
      ...
      {staff.length === 0 ? (
        <div className="glass-panel rounded-[2rem] border border-slate-200 py-24 text-center">
          ...
          <p className="header-font text-lg font-bold text-slate-500 uppercase tracking-[0.2em]">Добавьте сотрудников в разделе «Команда»</p>
        </div>
      ) : (
            ...
            >
              <div className="flex items-center gap-4 mb-8">
                ...
                <div className="min-w-0">
                  <h4 className="font-black text-slate-800 leading-tight tracking-tight truncate">{s.name}</h4>
                  <p className="text-[10px] text-slate-600 uppercase font-black tracking-widest mt-1 truncate">{s.role}</p>
                </div>
              </div>
              <div className="space-y-4 border-t border-slate-200 pt-6">
                <div className="flex justify-between text-xs gap-2">
                  <span className="text-slate-600 font-bold uppercase tracking-widest shrink-0">Выручка (подтв.)</span>
                  <span className="font-black text-slate-700 text-right">{rev.toLocaleString()} ₽</span>
                </div>
                <div className="flex justify-between text-xs">
                  <span className="text-slate-600 font-bold uppercase tracking-widest">Оклад</span>
                  <span className="font-black text-slate-700">{base.toLocaleString()} ₽</span>
                </div>
                <div className="flex justify-between text-xs">
