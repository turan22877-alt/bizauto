    ...
    <div className="space-y-10">
      <div>
        ...
        <p className="text-slate-600 font-medium">
          Центр взаимодействия с клиентами и сервисные уведомления
        </p>
      </div>
        ...
        <div className="glass-panel p-10 rounded-[2.5rem] border border-slate-200">
          ...
          <div className="space-y-4">
            {[
              ...
              <div
                key={i}
                className="flex items-center justify-between p-5 bg-slate-100 rounded-2xl hover:bg-orange-50 cursor-pointer transition-all border border-slate-200 hover:border-orange-500/30 group"
              >
                <div className="flex items-center gap-4">
                  <t.icon
                    size={20}
                    className="text-slate-500 group-hover:text-orange-600 transition-colors"
                  />
                  <span className="font-black text-sm text-slate-700 group-hover:text-orange-700 transition-colors">
                    {t.title}
                  </span>
                </div>
                <span
                  className={`text-[10px] font-black uppercase px-3 py-1 rounded-full ${t.status === "Активен" ? "bg-orange-500/10 text-orange-500" : "bg-slate-500/10 text-slate-500"}`}
                >
                  {t.status}
                </span>
              </div>
      ...
      <div className="glass-panel p-8 rounded-3xl border border-slate-200 flex items-center gap-6 bg-slate-100">
        ...
        <div>
          <p className="text-[10px] font-black uppercase text-slate-600 tracking-[0.2em] mb-1">
            System Advisory
          </p>
          <p className="text-sm font-bold text-slate-700">
            Все внешние уведомления требуют предварительной настройки API шлюза
