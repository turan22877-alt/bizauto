  ...
  const handleAddStaff = (e) => {
    ...
    const newStaff = {
      ...
      commissionPercent: Math.min(
        ...
        Math.max(
          0,
          parseInt(String(formData.get("commissionPercent") || "0"), 10) || 0,
        ),
      ),
      ownerUid,
    };
      ...
      <div className="flex justify-between items-end">
        <div>
          ...
          <p className="text-slate-600 font-medium">
            Команда специалистов высочайшего класса
          </p>
        </div>
        <Button
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
        {staff.length > 0 ? (
          staff.map((member) => (
            <div
              key={member.id}
              className="glass-panel p-8 rounded-[2.5rem] border border-slate-200 relative group hover:border-orange-500/30 transition-all"
            >
              <button
              ...
              <div className="space-y-4 pt-6 border-t border-slate-200">
                ...
                <div className="flex items-center justify-between text-xs">
                  ...
                  <span className="font-black text-slate-700">
                    {(member.baseSalary ?? 0).toLocaleString()} ₽ ·{" "}
                ...
                <div className="flex items-center justify-between text-xs">
                  ...
                  <span className="text-orange-500 font-black uppercase tracking-widest text-[9px]">
                    В сети
                  </span>
                </div>
              </div>
          ...
          <div className="lg:col-span-3 py-32 text-center opacity-20">
            ...
            <p className="header-font text-xl font-bold uppercase tracking-[0.4em]">
              Команда пуста
            </p>
          </div>
        )}
      ...
      >
        <form onSubmit={handleAddStaff} className="space-y-6">
          ...
          <div className="space-y-1">
            ...
            <select
              name="role"
              className="w-full bg-slate-100 border border-slate-200 rounded-2xl px-5 py-4 text-slate-800 outline-none focus:border-orange-500 transition-all appearance-none"
              required
            >
              <option className="bg-white" value="Ведущий специалист">
                Ведущий специалист
              </option>
              <option className="bg-white" value="Специалист">
                Специалист
              </option>
              <option className="bg-white" value="Менеджер">
                Менеджер
              </option>
              <option className="bg-white" value="Младший специалист">
                Младший специалист
              </option>
            </select>
          </div>
