  ...
  return (
    <div className="space-y-10">
      <div className="flex flex-col sm:flex-row sm:justify-between sm:items-end gap-4">
        <div>
          <h2 className="header-font text-3xl md:text-4xl font-black text-slate-800 mb-2">Финансы</h2>
          <p className="text-slate-600 font-medium">Суммы только из подтверждённых записей; сравнение — текущий и прошлый календарный месяц</p>
        </div>
      </div>
        ...
        <div className="space-y-6">
          ...
          <div className="glass-panel p-6 md:p-8 rounded-[1.75rem] border border-slate-200 flex items-center justify-between">
            <div className="flex items-center gap-4">
              ...
              <div>
                <p className="text-[10px] font-bold text-slate-600 uppercase tracking-widest">Средний чек</p>
      ...
      <div className="glass-panel rounded-[2rem] border border-slate-200 overflow-hidden">
        <div className="p-8 border-b border-slate-200 bg-slate-50">
          <h3 className="header-font text-lg font-bold text-slate-800">Подтверждённые операции</h3>
        </div>
            ...
            confirmed.map((app) => (
              <div key={app.id} className="p-6 md:p-8 flex items-center justify-between hover:bg-slate-50 transition-colors gap-4">
                ...
                <div className="text-right shrink-0">
                  <p className="font-black text-slate-800 text-lg">{app.price.toLocaleString()} ₽</p>
                  <p className="text-[10px] text-slate-600 uppercase font-black tracking-widest mt-1">{app.date}</p>
                </div>
              </div>
            ))
          ) : (
