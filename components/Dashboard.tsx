...
const Dashboard: React.FC<DashboardProps> = ({ clients, appointments, onNavigate }) => {
  ...
  const stats = [
    { label: 'Всего клиентов', value: clients.length, icon: Users, accent: 'from-green-500/20 to-transparent', iconClass: 'text-green-600' },
    ...
    <div className="space-y-10">
      <div className="grid grid-cols-1 xl:grid-cols-12 gap-8 items-stretch">
        <div className="xl:col-span-7 space-y-6">
          <div className="flex flex-col sm:flex-row sm:justify-between sm:items-end gap-4">
            <div>
              <h2 className="header-font text-3xl md:text-4xl font-black text-slate-800 mb-2 tracking-tight flex items-center gap-3">
                Обзор
                <Sparkles className="text-green-600 hidden sm:inline" size={28} />
              </h2>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-5 md:gap-6">
        {stats.map((stat, i) => (
          <div
            key={i}
            className="glass-panel card-3d p-6 md:p-8 rounded-[1.75rem] border border-slate-200 relative overflow-hidden group hover:border-green-500/25 transition-colors"
          >
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 lg:gap-8">
                ...
                .map((app) => (
                  ...
                  >
                    <div className="flex items-center gap-4 min-w-0">
                      <div className="w-10 h-10 bg-green-600/80 rounded-xl flex items-center justify-center font-bold text-white shrink-0">
                        {app.clientName[0]}
                      </div>
                    ...
                    <p className="text-xs font-black text-green-600 shrink-0 ml-2">{app.startTime}</p>
                  </div>
                ))}
            </div>
          ) : (
        ...
        <div className="glass-panel card-3d p-8 md:p-10 rounded-[2rem] border border-slate-200 bg-gradient-to-br from-green-600/[0.08] via-transparent to-violet-600/[0.06]">
          ...
          {completionRate === null ? (
            ...
            <div className="relative pt-1">
              <div className="flex mb-2 items-center justify-between">
                <span className="text-[10px] font-black uppercase rounded-full bg-green-600/15 text-green-700 px-3 py-1 border border-green-500/20">
                  Подтверждено / всего
                </span>
                <span className="text-xs font-black text-green-600">{completionRate}%</span>
              </div>
              <div className="overflow-hidden h-2.5 text-xs flex rounded-full bg-slate-200">
                <div
                  style={{ width: `${completionRate}%` }}
                  className="shadow-none flex flex-col text-center whitespace-nowrap text-white justify-center bg-gradient-to-r from-green-600 to-green-400 transition-all duration-500"
                />
              </div>
